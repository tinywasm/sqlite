...
func Open(dsn string) (*orm.DB, error) {
	...
	if err != nil {
		return nil, twfmt.Errf("failed to open sqlite database: %v", err)
	}

	if err := db.Ping(); err != nil {
		return nil, twfmt.Errf("failed to ping sqlite database: %v", err)
	}
...
func Close(db *orm.DB) error {
	if db == nil || db.RawExecutor() == nil {
		return twfmt.Err("database instance or executor is nil")
	}
	return db.Close()
}
